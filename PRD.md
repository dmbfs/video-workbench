...
> 版本 v1.0 · 模型目录与 API 契约已经官方文档实抓核实，调研记录见 `docs/RESEARCH.md`
> 形态：本地单机 Web 应用（localhost，无登录）· BYOK（用户自带模型 key）· 中文深色创作工作台
...
| M1 | monorepo 脚手架 + 设置页 + Mock 全链路 | 无 key 情况下：mock 生成 3 段 → 拼出可播放 mp4，截图自检通过 |
